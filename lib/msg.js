...
const fs = require('fs')
const { getBuffer, isUrl, generateButtonMessage } = require('./functions')
...
const sms = (conn, m) => {
	...
	m.sendButton = async (text, footer, buttons, imageUrl, options = {}) => {
		const message = generateButtonMessage(text, footer, buttons, imageUrl);
		return conn.sendMessage(m.chat, message, { quoted: m, ...options });
	};

	m.sendImageButton = async (image, text, footer, buttons, options = {}) => {
		let buffer;
